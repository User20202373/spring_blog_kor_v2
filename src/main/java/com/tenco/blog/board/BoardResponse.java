    ...
    public static class PageDTO {
        ...
        public PageDTO(Page<Board> page) {
            ...
            this.list = page.getContent().stream()
                    ...
                    .toList();
            // 사잔 지식 : page.getNumber() <- 0 번붙어 시작함
            // 화면에서는 변수 this.currentPage <-- 1번분터 보여줘야함
            this.currentPage = page.getNumber() + 1;
            this.size = page.getSize(); // 현재 default 값은 2임(수정가능)
            this.totalPages = page.getTotalPages();
            this.totalElements = page.getTotalElements(); // long로 설계되어 있음
            ...
            // 페이지 번호 윈도우 : 현재 페이지 기준 앞 뒤 2페이지 (최대 5개)
            // 예1 : 현재 페이지 5 -> [3,4,5,6,7]
            // 예2 : 현재 페이지 1 -> [1,2,3]

            int start = Math.max(1, this.currentPage - 2);

            // 예3 : 현재페이지 5, 총 페이지 5인경우 [3,4,5]
