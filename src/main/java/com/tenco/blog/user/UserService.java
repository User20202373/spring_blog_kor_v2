...
public class UserService {
    ...
    public User 회원정보수정(Integer id, UserRequest.UpdateDTO updateDTO) {
        ...
        if (updateDTO.getProfileImage() != null && !updateDTO.getProfileImage().isEmpty()) {
            // 새 프로필 정보 수정 요청
            // 1. 기존에 프로필 사진이 있다면 삭제하고 새로 저장(리스트), (DB수정)
        ...
        // 더티 체킹 활용
        userEntity.update(updateDTO,uuidImageFileName); // null이거나 새로운 이미지파일명 들어감
        return userEntity;
    }
    ...
    public User 프로필이미지삭제(Integer id) {
        ...
        //1차 캐쉬에 저장된 User 정보 수정 - 트랜잭션 종료시 반영(더티 체킹)
